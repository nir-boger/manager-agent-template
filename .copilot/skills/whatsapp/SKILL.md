...
6. **No signature, no joke (default).** WhatsApp is informal — don't append the Nirvana signature, don't squeeze in a one-liner. Send the message as written. **Single carve-out — `Your Team Group` group:** every outgoing message in that group ends with a Hebrew one-liner joke + the `WhatsAppGroupHe` signature variant from `_shared/signature.ps1` (the team knows about Nirvana — disclosure is mandatory there). All other 1:1 chats and groups, including `DM - חופשת לידה`, stay unsigned and joke-free. See §"Per-recipient style → Your Team Group" for the full rule.
...
| **Nir directly, but chat = `Your Team Group`** | **Auto-send (manual carve-out)** | **No.** | **No.** Runner fires `--confirm SEND` directly. Saved Nir preference 2026-05-04 — the team group goes out without preview/confirmation, with mandatory joke + `WhatsAppGroupHe` signature appended (see §"Per-recipient style → Your Team Group"). |
...
2. Compose in the right voice — every per-recipient rule from §"Per-recipient style" applies unchanged (load `spouse-or-partner-template` for Partner; Your Team wine/Reformer canon **plus** the mandatory end-of-message Hebrew joke + `WhatsAppGroupHe` signature appended via `Get-NirvanaSignatureText -Variant WhatsAppGroupHe`; maternity-group restraint; etc.). When tone is uncertain, run `read` first to match the thread.
